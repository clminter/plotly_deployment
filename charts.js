...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultFromSample = resultSample[0];
    console.log(resultFromSample);

    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otuIds = resultFromSample.otu_ids
    var otuIdsSliced = otuIds.slice(0,10).map(otuId => `OTU ${otuId}`).reverse();
    console.log(otuIdsSliced)

    var otuLables = resultFromSample.otu_labels;
    var otuLablesSliced = otuLables.slice(0,10).reverse();
    console.log(otuLablesSliced);

    var sampleValues = resultFromSample.sample_values;
    ...
    var yticks = otuIdsSliced.map(otuId => `${otuId}`);
    console.log(yticks);
    ...
    var gaugeData = [{
      ...
      gauge: {
        ...
        bar: { color: 'rgb(0, 102, 153)' },
        bgcolor: "white",
        borderwidth: 2,
        bordercolor: "gray",
        steps: [
          { range: [0, 2], color: 'rgb(255, 255, 153)'},
          { range: [2, 4], color: 'rgb(102, 255, 204)'},
          { range: [4, 6], color: "rgb(100, 255, 255)"},
          { range: [6, 8], color: "rgb(51, 204, 204)" },
          { range: [8, 10], color: "rgb(102, 153, 153)" },
        ]},
